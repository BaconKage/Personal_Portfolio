...
(() => {
  ...
  if (canvas && canvas.getContext) {
    ...
    let w = 0, h = 0, dpr, particles = [], raf = null, running = true;
    ...
    const resize = () => {
      ...
      if (prefersReduced) render(false); // redraw static constellation on resize
    };
    const LINK = 150;
    ...
    if (prefersReduced) {
      render(false); // static constellation — visible, but no motion
    } else {
      loop();
  ...
  (function aurora() {
    ...
    const opts = { antialias: false, alpha: false, depth: false, stencil: false,
      ...
      preserveDrawingBuffer: prefersReduced };
    let gl = null;
    ...
    // Reduced motion: render a single static aurora frame (colorful, no animation)
